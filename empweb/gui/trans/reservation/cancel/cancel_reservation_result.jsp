...
<div class="middle homepage">
  ...
  <c:choose>
    ...
    <c:when test="${empty fn:trim(param.reservation_id)}">
      <c:redirect url="index.jsp">
        <c:if test="${not empty fn:trim(param.reservation_id)}">
          <c:param name="reservation_id" value="${param.reservation_id}"/>
        </c:if>
    ...
    <c:otherwise>
      ...
      <h2><fmt:message key="cancel_reservation_result"/></h2>

      <trans:transResult>
        <trans:cancelReservationSingle
          reservationId="${fn:trim(param.reservation_id)}"
