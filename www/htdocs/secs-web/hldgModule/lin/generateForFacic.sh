...
$THIS_PATH/lin/registerTime.sh KEY generateForFacic_inicio
if [ -f $DB.txt ]
then
	$THIS_PATH/lin/registerTime.sh KEY generateForFacic_1
	$THIS_CISIS/mx null count=1 "pft=\`,'a$TAG_RESUMIDO{\`" now >> $proc_file

	$THIS_PATH/lin/registerTime.sh KEY generateForFacic_TXT2MST
	...
	$THIS_PATH/lin/registerTime.sh KEY generateForFacic_CHECK
	$THIS_CISIS/mx $DB lw=9999 "pft=@$THIS_PATH/checkv913.pft" now >> $PROC_FILE_CHECK
	if [ "@$DEBUG" == "@yes" ]
	then
		$THIS_PATH/lin/registerTime.sh KEY generateForFacic_DEBUG
		$THIS_CISIS/mx $DB lw=9999 "pft=@$THIS_PATH/present.pft" now >> $PROC_FILE_CHECK
	fi

	#$THIS_PATH/lin/registerTime.sh KEY generateForFacic_V2_INV
  	#$THIS_CISIS/mx $DB fst=@$THIS_PATH/temp.fst fullinv=$DB

	$THIS_PATH/lin/registerTime.sh KEY generateForFacic_V2_SEQ
	...
	$THIS_PATH/lin/registerTime.sh KEY generateForFacic_LOG_SEQ
  	echo Content of $myprocGetP.seq >> $LOG_FILE
  	cat $myprocGetP.seq >> $LOG_FILE

	$THIS_PATH/lin/registerTime.sh KEY generateForFacic_V2_CREATE3
  	$THIS_CISIS/mx "seq=$myprocGetP.seq" create=$myprocGetP now -all

	$THIS_PATH/lin/registerTime.sh KEY generateForFacic_V2_DISPLAY
  	$THIS_CISIS/mx $myprocGetP lw=9999 "proc='a9970{$TAG_RESUMIDO{'" "pft=@$THIS_PATH/display.pft" now >>   $proc_file

	$THIS_PATH/lin/registerTime.sh KEY generateForFacic_V2_DISPLAY_FIM
	$THIS_CISIS/mx null count=1 "pft=\`{',\`" now >> $proc_file

fi
$THIS_PATH/lin/registerTime.sh KEY generateForFacic_fim
