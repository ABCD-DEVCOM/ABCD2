...
chmod -R 775 $THIS_PATH/lin/*.sh

$THIS_PATH/lin/registerTime.sh $KEY generateForJournal_inicio
...
#
$THIS_PATH/lin/hldg_db.sh $THIS_CISIS $TITLE $KEY $TAG_RESUMIDO $HLDG $LOG_FILE $THIS_PATH $THIS_PROC_TMP_PATH
...
# $THIS_CISIS/mx $ALLFACIC btell=0 "$KEY=$" lw=99999 "proc='d920a920{',s('000000000000000000000000000000',v920),'{'"  "proc='d920a920{',mid(v920,size(v920)-30+1,30),'{'" "pft='echo \"',v920,'|',v911,'|',v912,'|',v913,'|',v914,'\">> $THIS_PROC_TMP_PATH/',if s(mpu,v913,mpl):'S' or s(mpu,v913,mpl):'P' or s(mpu,v913,mpl):'NE' or s(mpu,v913,mpl):'IN' or s(mpu,v913,mpl):'AN' then 'SPEC' else  'REG' fi,'.txt'/" now > $THIS_PROC_TMP_PATH/classify_facic.sh

# $THIS_PATH/lin/registerTime.sh $KEY generateForJournal_classifyRecords_execute
# chmod 775 $THIS_PROC_TMP_PATH/classify_facic.sh

# echo Executing $THIS_PROC_TMP_PATH/classify_facic.sh >> $LOG_FILE
# cat $THIS_PROC_TMP_PATH/classify_facic.sh >> $LOG_FILE
# $THIS_PROC_TMP_PATH/classify_facic.sh
...
then	
	...
	$THIS_PATH/lin/registerTime.sh $KEY generateForJournal_classifyRecords_generate
...
$THIS_PATH/lin/generateForFacic.sh $THIS_CISIS $THIS_PROC_TMP_PATH/ORD $THIS_PATH $PROC_FILE $LOG_FILE $TAG_RESUMIDO sep $PROC_FILE_CHECK $DEBUG

######################################
# Apply the proc
##
TIME_END=`date +%Y%m%d%H%M%S%N`
$THIS_PATH/lin/registerTime.sh $KEY generateForJournal_update_hldg
if [ -f $DO_TEMP_HLDG ]
then
...
$THIS_PATH/lin/registerTime.sh $KEY generateForJournal_update_hldg_fim
...
$THIS_PATH/lin/registerTime.sh $KEY generateForJournal_fim
#END