...
// SE LEE LOS PAR�METROS DE CONFIGURACI�N
...
global $db_path, $item_data, $arrHttp;
	...
	if (isset($arrHttp["year"]))    $ValorCapturado.="<17 0>".$arrHttp["year"]."</17>";       // A�O REVISTA
...
global $db_path,$arrHttp,$pft_totalitems,$pft_ni,$copies_title,$prefix_in,$formato_obj;
	...
	// pft_nc= pft para extraer el numero de clasificaci�n
...
//    echo $Expresion;
	...
	//se ubica el titulo en la base de datos de objetos de pr�stamo
	$Expresion=urlencode($Expresion);
...
foreach ($invent as $arrHttp["inventory"]){
	...
	//se extrae la informacion del numero de control del titulo y la base de datos catalogr�fica a la cual pertenece
		$tt=explode('||',$item);
		$control_number=$tt[0];
