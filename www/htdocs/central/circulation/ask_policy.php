...
// se leen las politicas de pr�stamo y la tabla de tipos de usuario
...
global $xWxis,$Wxis,$db_path,$msgstr,$wxisUrl;
	...
	// 30:Fecha reserva
	// 31:Hora de reserva
	// 40:Fecha l�mite de retiro
	// 60:Fecha de asignacion de la reserva
	// 130:Fecha de cancelaci�n de la reserva
	// 200:Fecha en que se ejecut� la reserva y se prest� el item al usuario
	...
	foreach ($contenido as $value){
		...
		if ($value!=""){
			...
			$fecha_reserva=$r[2];
			$hora_reserva=$r[3];
			$fecha_cancelacion=$r[6];  //Fecha en la cual el operador cancel� la reserva
			$fecha_limite=$r[4];      //Fecha hasta la cual la reserva asignada est� disponible
			$fecha_asignacion=$r[5];  //Fecha en la cual se asign� la reserva
			$fecha_prestamo=$r[7];    //Fecha en la cual se prest� el objeto reservado
			$status=$r[8];
	...
	if ($copies=="Y"){
		...
		//SE LEE EL PREFIJO A UTILIZAR PARA LOCALIZAR EL OBJETO A TRAV�S DE SU N�MERO DE INVENTARIO
	...
	//SI HAY EJEMPLARES DISPONIBLES VEMOS SI SE LE PUEDE PRESTAR AL USUARIO PORQUE EST� EN ALGUN LUGAR LA COLA DE RESERVAS ASIGNADAS
	...
	//SI HAY EJEMPLARES DISPONIBLES VEMOS SI SE LE PUEDE PRESTAR AL USUARIO PORQUE EST� EN ALGUN LUGAR LA COLA DE RESERVAS PENDIENTES
	if ($disponibilidad-1>0){
	...
	//SI EL USUARIO NO EST� EN NINGUNA DE LAS COLAS DE RESERVA
	...
	//SI QUEDAN EJEMPLARES DISPONIBLES, SE LE DA EL PR�STAMO AL USUARIO
	if ($disponibilidad>0){
		return array("continuar",0);
	}
...
$np=0;
$nv=0;
include("ec_include.php");  //se incluye el procedimiento para leer el usuario y los pr�stamos pendientes
...
if (trim($user)==""){
    ...
    if (isset($arrHttp["ecta"])){
    	if (!isset($reserve_active) or isset($reserve_active) and $reserve_active=="Y"){      //para ver si tiene activado el m�dulo de reservas. Se lee desde el abcd.def
...
//OJO AGREGARLE AL TIPO DE USUARIO SI SE LE PUEDEN PRESTAR CUANDO EST� VENCIDO
		...
		// Se localiza el registro catalogr�fico utilizando los datos anteriores
				$ref_cat=ReadCatalographicRecord($control_number,$catalog_db,$arrHttp["inventory"]);
	 			...
	 			if ($cont=="Y"){
					...
					$tt=explode('||',$tt[0]);
					$titulo=$tt[0];
					if (isset($arrHttp["year"]))    $titulo.=" A�o:".$arrHttp["year"];
					if (isset($arrHttp["volumen"])) $titulo.=" Volumen:".$arrHttp["volumen"];
					if (isset($arrHttp["numero"]))  $titulo.=" N�mero:".$arrHttp["numero"];
					if (isset($arrHttp["comments"]))
		    			$titulo.=" <font color=darkred>".$arrHttp["comments"]."</font>";
					$signatura=$tt[1];     //signatura topogr�fica
		    		$este_prestamo.= "<td bgcolor=white valign=top>$titulo</td>";
		    		$este_prestamo.= "<td bgcolor=white valign=top>";
		    		$grabar="Y";
					$este_prestamo.="</td>";

	// se verifica si el ejemplar est� prestado
					$tr_prestamos=LocalizarTransacciones($arrHttp["inventory"],"TR",$catalog_db);
					$items_prestados=count($tr_prestamos);
					$Opcion="";
					$msg="";
					$msg_1="";
					if (count($tr_prestamos)>0){   // Si ya existe una transacci�n de pr�stamo para ese n�mero de inventario, el ejemplar est� prestado
						$cont="N";
						$msg.="<font color=red>".$msgstr["itemloaned"]."<br></font>";
	        		}
					//SE VERIFICA SI EL USUARIO YA TIENE UN MISMO EJEMPLAR, VOLUMEN Y TOMO DE ESE T�TULO Y SI SE LE PERMITE O NO
				...
				//	$items_prestados=$var[1];
	        		...
	        		if ($cont=="Y"){
	        			...
	        			if ($grabar=="Y"){
	        				//SE LOCALIZA SI EL TITULO EST� RESERVADO
	        				if (!isset($reserve_active) or isset($reserve_active) and $reserve_active=="Y"){         //para ver si tiene activado el m�dulo de reservas se lee desde el abcd.def
