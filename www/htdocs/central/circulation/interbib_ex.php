...
global $db_path,$Wxis,$wxisUrl,$xWxis,$pr_loan,$pft_storobj,$recibo_arr,$recibo_list,$arrHttp,$ILL,$lang_db;
	...
	if (isset($item_data[8]))  // Informaci�n complementaria del item
...
// se lee la configuraci�n de la base de datos de objetos de pr�stamos
			...
			if (isset($obj[15])){
				...
				if (trim($fechal_usuario)!=""){
					if ($fecha_d>$fechal_usuario){
						$este_prestamo.= "fecha l�mite del usuario ";
						$norenovar="S";
						$cont="N";
						//die;
					}
			...
			if (isset($obj[15])){
				...
				if (trim($fechal_objeto)!=""){
					if ($fecha_d>$fechal_objeto){
						$este_prestamo.= "fecha l�mite del objeto ";
						$cont="N";
						$este_prestamo.="<hr>";
					}
				}
		...
		// Se localiza el registro catalogr�fico utilizando los datos anteriores
	 			...
	 			if ($cont=="Y"){
					...
					$signatura=$arrHttp["inventory"];     //signatura topogr�fica
...
//SE VERIFICA SI ALGUNO DE LOS EJEMPLARES DEVUELTOS EST� RESERVADO
...
//SE IMPRIMEN LOS RECIBOS DE PR�STAMOS
