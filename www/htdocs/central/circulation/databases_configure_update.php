...
if ($object_db!="loanobjects"){
	...
	if (!isset($arrHttp["item_inf_add"]))
		$arrHttp["item_inf_add"]="";
	echo "<h5><font color=darkred>". $msgstr["item_inf_add"]."</font></H5>";
	$Pft=stripslashes($arrHttp["item_inf_add"]);
	GuardarPft($Pft,$db_path.$arrHttp["base"]."/loans/".$_SESSION["lang"]."/item_inf_add.pft");

	if (!isset($arrHttp["pft_reserve"]))
		$arrHttp["pft_reserve"]="";
	echo "<h5><font color=darkred>". $msgstr["pft_reserve"]."</font></H5>";
	$Pft=stripslashes($arrHttp["pft_reserve"]);
	GuardarPft($Pft,$db_path.$arrHttp["base"]."/loans/".$_SESSION["lang"]."/pft_reserve.pft");
