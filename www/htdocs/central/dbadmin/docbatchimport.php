...
{ 
global $total,$tikapath,$converter_path,$db_path,$base_ant,$cisis_ver,$Wxis,$doctotal,$img_path,$OS,$procstartedatN,$docerror;
if(!is_dir($path.$archivo)){ 
//Get teh file info
...
if (filesize($path.$archivo)<26214400)
...
{
//Linux
$tikacommand='curl -T '.$img_path.$base_ant."/collection/".$fixpath.$newdocname.$total.".".$ext.' http://127.0.0.1:9998/tika --header "Accept: text/html" >'.$db_path."wrk/".$newdocname.$total.'.html';
exec($tikacommand,$outcm,$banderacm);
}
else
{
//Windows
$tikacommand='java -jar '.$tikapath.'tika.jar -h '.$img_path.$base_ant."/collection/".$fixpath.$newdocname.$total.".".$ext.' >'.$db_path."wrk/".$newdocname.$total.'.html';
exec($tikacommand,$outcm,$banderacm);
}
...
$temppath=substr($path,strpos($path,'ABCDImportRepo'));
$fixpath=substr($temppath,(strpos($temppath,'/')+1));
createPath($img_path.$base_ant."/collection/ImportError/");
createPath($img_path.$base_ant."/collection/ImportError/".$fixpath);
rename($path.$archivo, $img_path.$base_ant."/collection/ImportError/".$fixpath.$newdocname.$total.".".$ext);
}
...
if (is_dir($img_path.$base_ant."/collection/ImportError"))
{
//Move the ImportError to ABCDImportRepo	
rmdir($img_path.$base_ant."/collection/ABCDImportRepo");
rename($img_path.$base_ant."/collection/ImportError",$img_path.$base_ant."/collection/ABCDImportRepo");
}
