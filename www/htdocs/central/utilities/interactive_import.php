...
$truepathcol=$db_path."collections/".$base_ant."/";
...
<?php
echo "<p>Please press update to start</p>";  
...
// first we check the $factab and $fuctab as expected path-files in data-dir
 if ($unicode==1) {
         $factab=$db_path.$base_ant."/data/isisactab_utf8.tab";
         $fuctab=$db_path.$base_ant."/data/isisuctab_utf8.tab";
 }else {
         $factab=$db_path.$base_ant."/data/isisac.tab";
         $fuctab=$db_path.$base_ant."/data/isisuc.tab";
        }
// if these expected files do not exist we first look for isisac.tab and isisuc.tab in data-folder
 if (file_exists($factab)) {
      	$actab=$factab;
 }else{
//isisuctab is searched in general data-dir
    if (file_exists($db_path."isisac.tab"))
	$actab=$db_path."isisac.tab";
    }
// if no isisac.tab found we define ansi
//    if ($actab=="")  $uctab="ansi";


// if these expected files do not exist we first look for isisac.tab and isisuc.tab in data-folder
 if (file_exists($fuctab)) {
      	$uctab=$fuctab;
 }else{
//isisuctab is searched in general data-dir
...
// if no isisuc.tab found we define ansi
   if (($uctab=="") && ($actab=="")) $uctab="ansi";

}
$mxinv="";
if ($vdoctext!="") $mxinv=$converter_path." ".$db_path.$base_ant."/data/".$base_ant." fst=@".$db_path.$base_ant."/data/".$base_ant.".fst uctab=$uctab actab=$actab fullinv/m=".$db_path.$base_ant."/data/".$base_ant." now -all";
else
$mxinv=$converter_path." ".$db_path.$base_ant."/data/".$base_ant." fst=@".$db_path.$base_ant."/data/external.fst uctab=$uctab actab=$actab fullinv/m=".$db_path.$base_ant."/data/".$base_ant." now -all";
