<?php
/*
20220320 fho4abcd constants for config path, renamed from configure to inc_barcode_configure
*/
$copies="";
$archivo=$db_path.$configfileprefix.$arrHttp["tipo"].$configfilesuffix;
$msg_err="";
if (!file_exists($archivo)){
	 $msg_err=$msgstr["barcode_conf"]." (".$configfileprefix.$arrHttp["tipo"].$configfilesuffix.")";
}else{
	$fp=file($archivo);
	foreach ($fp as $value){
		$value=trim($value);
