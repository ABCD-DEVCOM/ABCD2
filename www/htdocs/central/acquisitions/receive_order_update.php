...
if ($arrHttp["typeofobj"]=="N"){   // si el objeto es nuevo y el n�mero de objeto no existe en la base de datos
...
//IF THE PURCHASE ORDER WAS GENERATED FROM A SUGGESTIONS, GET THE SUGGESTION FOR RECOLECTING THE PRE-CATALOGATION
	...
	if (isset($arrHttp["suggestion"])){
		$res=EjecutarBusqueda("CN_".$arrHttp["suggestion"],"suggestions");     // se localiza la recomendaci�n para sacar los datos de pre-catalogacion
		if ($res==0){
...
//IF THERE IS NO SUGGESTION GET THE DATA FROM THE PURCHASEORDER DATABASE
        $res=EjecutarBusqueda("ON_".$arrHttp["order"],"purchaseorder");     // se localiza la recomendaci�n para sacar los datos de pre-catalogacion
		if ($res==0){
...
global $db_path;
	...
	//se protege el archivo con el n�mero secuencial
		chmod($archivo,0555);
	// se lee el �ltimo n�mero asignado y se le agrega 1
		$fp=file($archivo);
		$cn=implode("",$fp);
		$cn=$cn+1;
