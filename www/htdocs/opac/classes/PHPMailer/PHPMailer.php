...
class PHPMailer
{
     ...
     * @param string $meta_encoding The charset to use when decoding the address list string.
     *
     * @return array
     */
    public static function parseAddresses($addrstr, $useimap = true, $meta_encoding = self::CHARSET_ISO88591)
    {
        $addresses = [];
            ...
            foreach ($list as $address) {
                if (
                    ...
                    if (
                        ...
                        mb_internal_encoding($meta_encoding);
    ...
    public function encodeHeader($str, $position = 'text')
    {
        ...
        if ($this->has8bitChars($str)) {
            $meta_encoding = $this->CharSet;
        } else {
            $meta_encoding = static::CHARSET_ASCII;
        }

        //Q/B encoding adds 8 chars and the charset ("` =?<charset>?[QB]?<content>?=`").
        $overhead = 8 + strlen($meta_encoding);
        ...
        switch ($encoding) {
            case 'B':
                ...
                $encoded = preg_replace('/^(.*)$/m', ' =?' . $meta_encoding . "?$encoding?\\1?=", $encoded);
                break;
            case 'Q':
